...
class googlenet():
    ...
    def _conv_def(self, filters, kernel_size, *, strides = 1, prev_layers = None):
    ...
    def _Inception(self, prev_layers, conv1_1, conv13_1, conv13_3, conv15_1, conv15_5, convp1_1):
        
        #(1 x 1)
        conv11_l = self._conv_def(filters = conv1_1, kernel_size = 1, prev_layers = prev_layers)
        
        #(1 x 3)
        conv13_l = self._conv_def(filters = conv13_1, kernel_size = 1, prev_layers = prev_layers)
        conv13_3_l = self._conv_def(filters = conv13_3, kernel_size = 3, prev_layers = conv13_l)
        
        #(1 x 5)
        conv15_l = self._conv_def(filters = conv15_1, kernel_size = 1, prev_layers = prev_layers)
        conv15_5_l = self._conv_def(filters = conv15_5, kernel_size = 5, prev_layers = conv15_l)
        
        #(1 x pool)
        convp1_l = MaxPooling2D(pool_size = 3, strides = 1, padding = self.padding)(prev_layers)
        convp1_1_l = self._conv_def(filters = convp1_1, kernel_size = 1, prev_layers = convp1_l)
    ...
    def mini_Inception(self, prev_layers, conv1_1, conv3_1):
        conv11 = self._conv_def(filters = conv1_1, kernel_size = 1, prev_layers = prev_layers)
        conv33 = self._conv_def(filters = conv3_1, kernel_size = 3, prev_layers = prev_layers)
        
        inception_out = concatenate([conv11, conv33], axis = -1)
        
        return inception_out
    
    def _downsample(self, prev_layers, filters):
        
        conv33 = self._conv_def(filters = filters, kernel_size = 3, strides = 2, prev_layers = prev_layers)
    ...
    def build_model(self, *, mini = False):
        ...
        #Arguments
            ...
            mini (boolean) : boolean value to specify which version of googlenet is built among original and mini version of googlenet
        ...
        Orgiginal version - https://arxiv.org/pdf/1409.4842.pdf
        mini version - https://arxiv.org/pdf/1611.03530.pdf - Figure 3
        ------------------------------------------------------------------------------
        ...
        if mini is False:
            ...
            conv1 = self._conv_def(filters = 64, kernel_size = 7, strides = 2, prev_layers = input_layers)
            conv1_pool = MaxPooling2D(pool_size = 3, strides = 2, padding = 'same')(conv1)
            
            conv2_1 = self._conv_def(filters = 64, kernel_size = 1, strides = 1, prev_layers = conv1_pool)
            conv2 = self._conv_def(filters = 192, kernel_size = 3, strides = 1, prev_layers = conv2_1)
            conv2_pool = MaxPooling2D(pool_size = 3, strides = 2, padding = 'same')(conv2)
            
            inception_3a = self._Inception(prev_layers = conv2_pool,
                                         conv1_1 = 64, conv13_1 = 96, conv13_3 = 128, conv15_1 = 16, conv15_5 = 32, convp1_1 = 32)
            inception_3b = self._Inception(prev_layers = inception_3a,
            ...
            inception_4a = self._Inception(inception_3_pool,
                                         192, 96, 208, 16, 48, 64)
            inception_4b = self._Inception(inception_4a,
                                         160, 112, 224, 24, 64, 64)
            inception_4c = self._Inception(inception_4b,
                                         128, 128, 256, 24, 64, 64)
            inception_4d = self._Inception(inception_4c,
                                         112, 144, 144, 32, 64, 64)
            inception_4e = self._Inception(inception_4d,
                                         256, 160, 320, 32, 128, 128)
            inception_4_pool = MaxPooling2D(pool_size = 3, strides = 2, padding = 'same')(inception_4e)
            
            inception_5a = self._Inception(inception_4_pool,
                                         256, 160, 320, 32, 128, 128)
            inception_5b = self._Inception(inception_5a,
        ...
        if mini:
            ...
            conv1 = self._conv_def(filters = 96, kernel_size = 3, prev_layers = input_layers)
            
            inception_3a = self._mini_Inception(prev_layers = conv1,
                                              conv1_1 = 32, conv3_1 = 32)
            inception_3b = self._mini_Inception(inception_3a,
                                              32, 48)
            
            downsample_1 = self._downsample(prev_layers = inception_3b, filters = 80)
            
            inception_4a = self._mini_Inception(downsample_1,
                                              112, 48)
            inception_4b = self._mini_Inception(inception_4a,
                                              96, 64)
            inception_4c = self._mini_Inception(inception_4b,
                                              80, 80)
            inception_4d = self._mini_Inception(inception_4c,
                                              48, 96)
            
            downsample_2 = self._downsample(inception_4d, 96)
            
            inception_5a = self._mini_Inception(downsample_2,
                                              176, 160)
            inception_5b = self._mini_Inception(inception_5a,
                                              176, 160)
...
googlenet_mini = googlenet(input_shape = (32,32,3), y_shape = 10, activation = 'relu', num_class = 10,
                          epochs = 5)
